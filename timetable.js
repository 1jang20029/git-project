    ...
    courses.forEach(course => {
        course.times.forEach(t => {
            // 주말 숨김 설정 확인
            const maxDays = settings.showWeekend ? 6 : 5;
            if (t.day > maxDays) return;

            // 해당 요일과 교시의 셀 찾기
            const targetCell = container.querySelector(`.class-cell[data-day="${t.day}"][data-period="${t.start}"]`);
            ...
            // 높이 계산 (시작 교시부터 종료 교시까지의 높이)
            const spanPeriods = t.end - t.start + 1;
    ...
    function saveCourse(event) {
        ...
        // 2) 같은 과목 내에서 시간대 겹침 검사
        for (let i = 0; i < slots.length; i++) {
            for (let j = i + 1; j < slots.length; j++) {
                // 같은 요일이고
                if (slots[i].day === slots[j].day) {
                    // 시간대가 겹치면 (interval overlap)
