...
function initNaverMap() {
  ...
  // 4) 강제 리사이즈 + 안전한 refresh
  window.dispatchEvent(new Event('resize'));
  setTimeout(() => {
    try { naverMap.refresh(); console.log('지도 refresh 완료'); }
    catch (e) { console.warn('refresh 건너뜀:', e); }
  }, 500);

  // 5) 스타일맵 초기화 후 한 번 더 refresh
