...
async function loadFacilities() {
  try {
    ...
    console.error('시설 데이터 로드 실패:', err);
    renderFacilities([]);
  }
}

// ─────────── renderFacilities: 시설 카드 렌더링 (추가) ───────────
function renderFacilities(facilities) {
  ...
  facilities.forEach((f) => {
    const card = document.createElement('div');
    card.className = 'building-card'; // 동일한 스타일 사용
    card.innerHTML = `
      <h3 class="building-name">${f.name}</h3>
      <p class="building-desc">${f.address || ''}</p>
      <div class="building-actions">
        <button class="btn btn-primary" data-id="${f.id}" onclick="showFacilityOnMap(${f.latitude}, ${f.longitude})">
          📍 지도에서 보기
        </button>
        <button class="btn btn-outline" data-id="${f.id}" onclick="viewFacilityPhotos(${f.id})">
          🖼️ 사진 보기
        </button>
      </div>
    `;
...
// ─────────── showFacilityOnMap: 시설 지도에서 보기 (추가) ───────────
function showFacilityOnMap(lat, lng) {
  if (naverMap) {
...
// ─────────── viewFacilityPhotos: 시설 사진 모달 띄우기 (추가) ───────────
async function viewFacilityPhotos(facilityId) {
  try {
    const res = await fetch(`/api/facilities/${facilityId}/photos`);
    if (!res.ok) throw new Error('사진 데이터 로드 실패');
...
// ─────────── loadBuildingData: (세부적으로 분리 가능) ───────────
// 이미 loadBuildings(), renderBuildings(), addMapMarkers()로 구현되어 있음.
