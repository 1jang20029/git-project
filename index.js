...
function startProfileEdit() {
    ...
    console.log('프로필 수정 모드 시작 - 원본 데이터 백업 중...');
    ...
    originalProfileBackup = {
        ...
        timestamp: Date.now() // 백업 시점 기록
    };
    
    // 프로필 수정 모드 활성화
    isProfileEditMode = true;
    
    console.log('원본 프로필 데이터 백업 완료:', originalProfileBackup);
...
function setupPageLeaveWarning() {
    ...
    // 현재 URL을 히스토리에 추가 (뒤로가기 감지용)
    history.pushState({ profileEdit: true }, '', window.location.href);
}


// 페이지 이탈 핸들러
function handlePageLeave(event) {
    if (isProfileEditMode && originalProfileBackup) {
...
function handleBackButton(event) {
    if (isProfileEditMode && originalProfileBackup) {
        ...
        const shouldLeave = confirm('변경사항이 저장되지 않았습니다. 페이지를 떠나시겠습니까?');
        if (shouldLeave) {
            // 취소 처리 후 이동
            cancelProfileEdit();
        } else {
...
function cancelProfileEdit() {
    ...
    if (!originalProfileBackup) {
        console.warn('백업된 원본 데이터가 없습니다. 메인으로 이동합니다.');
        window.location.href = 'index.html';
        return false;
    }
    
    console.log('프로필 변경사항 취소 - 원본 데이터로 복원 중...');
    
    try {
        // 원본 데이터로 완전히 복원
        localStorage.setItem(`user_${currentUser}_name`, originalProfileBackup.name);
        localStorage.setItem(`user_${currentUser}_department`, originalProfileBackup.department);
        localStorage.setItem(`user_${currentUser}_grade`, originalProfileBackup.grade);
        localStorage.setItem(`user_${currentUser}_profileImageType`, originalProfileBackup.profileImageType);
        localStorage.setItem(`user_${currentUser}_profileImage`, originalProfileBackup.profileImage);
        localStorage.setItem(`user_${currentUser}_studentId`, originalProfileBackup.studentId);
        
        // 커스텀 프로필 이미지 처리
        if (originalProfileBackup.customProfileImage) {
            localStorage.setItem(`user_${currentUser}_customProfileImage`, originalProfileBackup.customProfileImage);
        } else {
            localStorage.removeItem(`user_${currentUser}_customProfileImage`);
        }
        ...
        // 취소 완료 메시지 설정
        localStorage.setItem('profileActionMessage', JSON.stringify({
            type: 'cancel',
            message: '프로필 변경이 취소되었습니다.',
            timestamp: Date.now()
        }));
...
function saveProfileEdit() {
    ...
    try {
        // 여기서 실제 프로필 저장 로직 실행
        // (기존의 프로필 저장 코드가 여기에 들어갑니다)
        
        // 저장 성공 시 프로필 수정 모드 종료
        endProfileEdit();
...
// 5. 프로필 수정 모드 종료
// ========================================
function endProfileEdit() {
    console.log('프로필 수정 모드 종료');
    
    // 백업 데이터 초기화
    originalProfileBackup = null;
...
function triggerProfileUpdate() {
    ...
    // 메인 페이지의 updateAllProfileImages 함수 실행 (있다면)
...
function hasProfileChanges() {
    const currentUser = localStorage.getItem('currentLoggedInUser');
    if (!currentUser || !originalProfileBackup) {
        return false;
    }
    ...
    // 원본과 비교
    const hasChanges = 
        currentData.name !== originalProfileBackup.name ||
        currentData.department !== originalProfileBackup.department ||
        currentData.grade !== originalProfileBackup.grade ||
        currentData.profileImageType !== originalProfileBackup.profileImageType ||
        currentData.profileImage !== originalProfileBackup.profileImage ||
        currentData.customProfileImage !== originalProfileBackup.customProfileImage ||
        currentData.studentId !== originalProfileBackup.studentId;
    
    console.log('변경사항 확인:', hasChanges);
    console.log('현재 데이터:', currentData);
    console.log('원본 데이터:', originalProfileBackup);
...
// 9. 메인 페이지에서 메시지 표시
// ========================================
function showProfileActionMessage() {
    const messageData = localStorage.getItem('profileActionMessage');
    if (messageData) {
        try {
            const message = JSON.parse(messageData);
            
            // 5초 이내의 메시지만 표시 (중복 방지)
            if (Date.now() - message.timestamp < 5000) {
                alert(message.message);
            }
            
            // 메시지 삭제
            localStorage.removeItem('profileActionMessage');
        } catch (e) {
...
function initializeProfileEditPage() {
    ...
    cancelButtons.forEach(button => {
        // 취소 관련 텍스트가 있는 버튼만 처리
        if (button.textContent.includes('취소') || 
            button.textContent.includes('Cancel') ||
            button.textContent.includes('닫기') ||
            button.textContent.includes('되돌리기')) {
            
            button.onclick = function(event) {
                event.preventDefault();
                handleCancelButton();
            };
            console.log('취소 버튼 이벤트 리스너 등록:', button.textContent);
        }
    });
    ...
    saveButtons.forEach(button => {
        ...
        if (button.textContent.includes('저장') || 
            ...
            console.log('저장 버튼 이벤트 리스너 등록:', button.textContent);
        }
    });
...
// 기존 updateAllProfileImages 함수 확장
const originalUpdateAllProfileImages = typeof updateAllProfileImages !== 'undefined' ? 
    updateAllProfileImages : function() {};

function updateAllProfileImages() {
    // 기존 함수 실행
    originalUpdateAllProfileImages();
    ...
    if (window.location.pathname.includes('index.html') || 
        ...
        window.location.pathname === '') {
        document.addEventListener('DOMContentLoaded', function() {
            setTimeout(showProfileActionMessage, 500);
    });
}
}
...
if (window.location.pathname.includes('profile-edit.html')) {
    document.addEventListener('DOMContentLoaded', function() {
        setTimeout(initializeProfileEditPage, 100);
    });
}
...
// ========================================
if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
    window.profileEditDebug = {
        // 백업 상태 확인
        checkBackup: function() {
            console.log('백업 데이터:', originalProfileBackup);
            console.log('수정 모드:', isProfileEditMode);
        },
    ...
    console.log('🔧 프로필 수정 디버그 도구: window.profileEditDebug');
}

// ========================================
// 14. 전역 함수로 export (다른 스크립트에서 사용 가능)
// ========================================
window.profileEditSystem = {
    start: startProfileEdit,
