...
const createRestaurantCard = function(restaurant) {
    ...
    likeButton.addEventListener('click', function(e) {
        e.stopPropagation();
        const result = toggleReaction(restaurant.id, 'like');
        if (result) {
            // 버튼 활성화/비활성화 상태 업데이트
            ...
            // 좋아요 수 업데이트
            const likeCountElement = this.closest('.card-content').querySelector('.like-bubble span');
            if (likeCountElement) {
                likeCountElement.textContent = result.restaurant.likes;
            }
        }
    });
    ...
    starButton.addEventListener('click', function(e) {
        e.stopPropagation();
        const result = toggleReaction(restaurant.id, 'star');
        if (result) {
            // 버튼 활성화/비활성화 상태 업데이트
            if (result.hasReacted) {
                this.classList.remove('active');
            ...
            // 추천 수 업데이트
            const starCountElement = this.closest('.card-content').querySelector('.star-bubble span');
            if (starCountElement) {
                starCountElement.textContent = result.restaurant.stars;
            }
        }
    });
    ...
    dislikeButton.addEventListener('click', function(e) {
        e.stopPropagation();
        const result = toggleReaction(restaurant.id, 'dislike');
        if (result) {
            // 버튼 활성화/비활성화 상태 업데이트
            if (result.hasReacted) {
                this.classList.remove('active');
            ...
            // 싫어요 수 업데이트
            const dislikeCountElement = this.closest('.card-content').querySelector('.dislike-bubble span');
            if (dislikeCountElement) {
                dislikeCountElement.textContent = result.restaurant.dislikes;
            }
        }
    });
