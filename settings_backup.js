...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 2) localStorage에서 이전 설정 값을 가져와서 각 요소 초기화
  ...
  // 3) UI 표시/숨김: “자동” 모드일 때만 time-picker 보이기
  ...
  // 4) 테마 적용 함수: 현재 시간에 맞춰 라이트/다크 모드로 설정
  function applyThemeAccordingToSchedule() {
    ...
    if (mode === 'light') {
      // 항상 라이트 모드 → body.light-mode 클래스를 제거
      document.body.classList.remove('light-mode');
    } else if (mode === 'dark') {
      // 항상 다크 모드 → body.light-mode 클래스를 추가하지 않음 (Dark 기본)
      document.body.classList.add('light-mode'); // 만약 기본이 라이트라면, 반대로 적용 필요
    } else {
      // mode === 'auto' → 시간대별 자동
      if (lightStartTime.value < darkStartTime.value) {
        // 예시: 07:00 < 19:00 → 낮(07~19)은 라이트, 밤(19~24, 00~07)은 다크
        ...
        // 예시: 19:00 < 07:00 → 밤(19~24, 00~07)은 다크, 낮(07~19)은 라이트
