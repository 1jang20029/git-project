...
function register() {
    ...
    if (!validateStudentId(studentId)) {
        ...
        document.getElementById('studentId-error').textContent = '10자리 숫자로 입력해주세요 (예: 2024123456)';
        return;
    }
    ...
    // 사용자 정보를 localStorage에 저장 (학생 권한으로만 저장)
    ...
    localStorage.setItem(`user_${userId}_role`, 'student'); // 학생 권한으로만 설정
    localStorage.setItem(`user_${userId}_grade`, selectedGrade);
    localStorage.setItem(`user_${userId}_role_status`, 'approved'); // 학생은 바로 승인
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    idInput.addEventListener('input', function() {
        ...
        if (this.value && !validateStudentId(this.value)) {
            errorDiv.style.display = 'block';
            errorDiv.textContent = '10자리 숫자로 입력해주세요 (예: 2024123456)';
        } else {
            errorDiv.style.display = 'none';
        }
    });
